...
def filter_by_state(input_list: List[Dict[str, Any]], state: str = "EXECUTED") -> List[Dict[str, Any]]:
    ...
    return [item for item in input_list if state == item.get("state")]
...
def sort_by_price_in_cat(input_list: List[Dict[str, Any]], cat: Optional[str] = None) -> List[Dict[str, Any]]:
    """Returns sorted list of dicts in specified category"""
    categories = [x.get("category") for x in input_list]
    if cat is None:
...
def orders_info(orders: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    """Returns dict of average order value and number of orders for each month"""
    monthly_data: Dict[str, Dict[str, float]] = dict()
    result_data: Dict[str, Dict[str, float]] = dict()
    for order in orders:
        ...
        if year_month not in monthly_data:
            monthly_data[year_month] = {"total_price": 0.0, "order_count": 0.0}
    ...
    for month, stats in monthly_data.items():
        average_order_value = stats["total_price"] / stats["order_count"] if stats["order_count"] > 0 else 0.0
